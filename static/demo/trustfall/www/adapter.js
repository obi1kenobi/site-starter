import init, { Schema, initialize, executeQuery } from "../www2/trustfall_wasm.js";
...
export class MyAdapter {
    ...
    *projectNeighbors(data_contexts, current_type_name, edge_name, parameters) {
        if (current_type_name === "HackerNewsStory" ||
            current_type_name === "HackerNewsJob" ||
            current_type_name === "HackerNewsComment") {
            if (edge_name === "link") {
                for (const ctx of data_contexts) {
...
function* lazyFetchMap(fetchPort, inputs, func) {
    if (inputs) {
...
let _adapterFetchChannel;
let _resultIter;
function performQuery(query, args) {
